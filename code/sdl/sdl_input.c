...
===============
*/
static void IN_PrintKey( const SDL_KeyboardEvent *keyEvent, keyNum_t key, qboolean down )
{
	if( down )
		Com_Printf( "+ " );
	else
	...
	Com_Printf( "Scancode: 0x%02x(%s) Key: 0x%02x(%s)",
			keyEvent->scancode, SDL_GetScancodeName( keyEvent->scancode ),
			keyEvent->key, SDL_GetKeyName( keyEvent->key ) );

	if( keyEvent->mod & SDL_KMOD_LSHIFT )   Com_Printf( " SDL_KMOD_LSHIFT" );
	if( keyEvent->mod & SDL_KMOD_RSHIFT )   Com_Printf( " SDL_KMOD_RSHIFT" );
	if( keyEvent->mod & SDL_KMOD_LCTRL )    Com_Printf( " SDL_KMOD_LCTRL" );
	if( keyEvent->mod & SDL_KMOD_RCTRL )    Com_Printf( " SDL_KMOD_RCTRL" );
	if( keyEvent->mod & SDL_KMOD_LALT )     Com_Printf( " SDL_KMOD_LALT" );
	if( keyEvent->mod & SDL_KMOD_RALT )     Com_Printf( " SDL_KMOD_RALT" );
	if( keyEvent->mod & SDL_KMOD_LGUI )     Com_Printf( " SDL_KMOD_LGUI" );
	if( keyEvent->mod & SDL_KMOD_RGUI )     Com_Printf( " SDL_KMOD_RGUI" );
	if( keyEvent->mod & SDL_KMOD_NUM )      Com_Printf( " SDL_KMOD_NUM" );
	if( keyEvent->mod & SDL_KMOD_CAPS )     Com_Printf( " SDL_KMOD_CAPS" );
	if( keyEvent->mod & SDL_KMOD_MODE )     Com_Printf( " SDL_KMOD_MODE" );
	if( keyEvent->mod & SDL_KMOD_SCROLL )   Com_Printf( " SDL_KMOD_SCROLL" );
...
===============
*/
static keyNum_t IN_TranslateSDLToQ3Key( const SDL_KeyboardEvent *keyEvent, qboolean down )
{
	keyNum_t key = 0;
	...
	if( keyEvent->scancode >= SDL_SCANCODE_1 && keyEvent->scancode <= SDL_SCANCODE_0 )
	{
		...
		if( keyEvent->scancode == SDL_SCANCODE_0 )
			key = '0';
		else
			key = '1' + keyEvent->scancode - SDL_SCANCODE_1;
	}
	else if( keyEvent->key >= SDLK_SPACE && keyEvent->key < SDLK_DELETE )
	{
	...
	else
	{
		switch( keyEvent->key )
		{
			...
			default:
				if( !( keyEvent->key & SDLK_SCANCODE_MASK ) && keyEvent->scancode <= 95 )
				{
					...
					// to SDL 1.2.
					key = K_WORLD_0 + (int)keyEvent->scancode;
				}
				break;
		}
	}
...
struct
{
	qboolean buttons[SDL_GAMEPAD_BUTTON_COUNT];
...
===============
*/
static keyNum_t IN_KeyFor( SDL_GamepadButton sdlButton )
{
	switch(sdlButton)
	{
		case SDL_GAMEPAD_BUTTON_SOUTH:          return K_PAD0_A;
		case SDL_GAMEPAD_BUTTON_EAST:           return K_PAD0_B;
		case SDL_GAMEPAD_BUTTON_WEST:           return K_PAD0_X;
		case SDL_GAMEPAD_BUTTON_NORTH:          return K_PAD0_Y;
		case SDL_GAMEPAD_BUTTON_BACK:           return K_PAD0_BACK;
		case SDL_GAMEPAD_BUTTON_GUIDE:          return K_PAD0_GUIDE;
		case SDL_GAMEPAD_BUTTON_START:          return K_PAD0_START;
		case SDL_GAMEPAD_BUTTON_LEFT_STICK:     return K_PAD0_LEFTSTICK_CLICK;
		case SDL_GAMEPAD_BUTTON_RIGHT_STICK:    return K_PAD0_RIGHTSTICK_CLICK;
		case SDL_GAMEPAD_BUTTON_LEFT_SHOULDER:  return K_PAD0_LEFTSHOULDER;
		case SDL_GAMEPAD_BUTTON_RIGHT_SHOULDER: return K_PAD0_RIGHTSHOULDER;
		case SDL_GAMEPAD_BUTTON_DPAD_UP:        return K_PAD0_DPAD_UP;
		case SDL_GAMEPAD_BUTTON_DPAD_DOWN:      return K_PAD0_DPAD_DOWN;
		case SDL_GAMEPAD_BUTTON_DPAD_LEFT:      return K_PAD0_DPAD_LEFT;
		case SDL_GAMEPAD_BUTTON_DPAD_RIGHT:     return K_PAD0_DPAD_RIGHT;
		case SDL_GAMEPAD_BUTTON_MISC1:          return K_PAD0_MISC1;
		case SDL_GAMEPAD_BUTTON_RIGHT_PADDLE1:	return K_PAD0_PADDLE1;
		case SDL_GAMEPAD_BUTTON_LEFT_PADDLE1:   return K_PAD0_PADDLE2;
		case SDL_GAMEPAD_BUTTON_RIGHT_PADDLE2:  return K_PAD0_PADDLE3;
		case SDL_GAMEPAD_BUTTON_LEFT_PADDLE2:   return K_PAD0_PADDLE4;
		case SDL_GAMEPAD_BUTTON_TOUCHPAD:       return K_PAD0_TOUCHPAD;
		case SDL_GAMEPAD_BUTTON_MISC2:          return K_PAD0_MISC2;
		case SDL_GAMEPAD_BUTTON_MISC3:          return K_PAD0_MISC3;
		case SDL_GAMEPAD_BUTTON_MISC4:          return K_PAD0_MISC4;
		case SDL_GAMEPAD_BUTTON_MISC5:          return K_PAD0_MISC5;
		case SDL_GAMEPAD_BUTTON_MISC6:          return K_PAD0_MISC6;
...
static void IN_GamepadMove( void )
{
	...
	{
		int axis = SDL_GetGamepadAxis(gamepad, SDL_GAMEPAD_AXIS_LEFTX + i);
