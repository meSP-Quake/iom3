...
void vk_createSwapChain(VkDevice device, VkSurfaceKHR surface, VkSurfaceFormatKHR surface_format)
{
...

    {
        ...
        // Re-query surface capabilities to get fresh values.
        // The initial query in vk_instance.c may have returned stale dimensions
        // (e.g., on Wayland the surface may not have been fully realized yet).
        // This matches Quake3e's approach of querying right before swapchain creation.
        VK_CHECK(qvkGetPhysicalDeviceSurfaceCapabilitiesKHR(vk.physical_device, surface, &vk.surface_caps));

        VkExtent2D image_extent;

        // Use currentExtent when the compositor dictates the size (typical on X11).
        // When currentExtent is 0xFFFFFFFF, the app must choose its own extent
        // (typical on Wayland) - fall back to glConfig dimensions clamped to the
        // surface's supported range.
        if (vk.surface_caps.currentExtent.width != 0xFFFFFFFF &&
            vk.surface_caps.currentExtent.height != 0xFFFFFFFF) {
            image_extent = vk.surface_caps.currentExtent;
        } else {
            int winWidth, winHeight;
            R_GetWinResolution(&winWidth, &winHeight);
            image_extent.width = MIN(vk.surface_caps.maxImageExtent.width,
                MAX(vk.surface_caps.minImageExtent.width, (uint32_t)winWidth));
            image_extent.height = MIN(vk.surface_caps.maxImageExtent.height,
                MAX(vk.surface_caps.minImageExtent.height, (uint32_t)winHeight));
        }
